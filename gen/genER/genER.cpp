...
int main() {
    for (int N = 30; N < 200; ++N) {
        if (N < 1) return 0;
        std::ofstream ofs;
        ofs.open("ER" + std::string(N < 100 ? "0" : "") + std::to_string(N) + ".gr");
        ...
        for (int i = 0; i < N; ++i) {
            int x = ri[N]->get();
            int y = ri[N]->get();
            if (x == y) y = (x + 1) % ids.size();
            if (x > y) swap(x, y);
            for (int j = 0; j < edges.size(); ++j) {
                if (edges[j].first == x && edges[j].second == y) {
                    x = -1;
                    y = -1;
                    break;
                }
            }
            if (x != -1) edges.push_back({x, y});
        }
        ofs << "p tdp " << N << " " << edges.size() << endl;
        for (auto edge : edges) {
